...
import { AgentQueueStatsWrapper, CustomDataTable } from './AgentQueueStatsStyles';
...
const AgentQueueStatsView = () => {
  ...
  useEffect(() => {
    const fetchQueues = async () => {
      ...
      if (isAgent && workerSid && workspaceSid && functionUrl) {
        try {
          ...
          setAgentAssignedQueueSids(data.queueSids || []);
        } catch (error) {
          console.error('[agent-queue-stats] Failed to fetch agent queues:', error);
        }
      }
    };
  ...
  const keysToFilter =
    filterValue === 'Invisalign'
      ...
      : filterValue === 'iTero'
        ? KEYS_ITERO
        : KEYS_INVISALIGN.concat(KEYS_ITERO);
      ...
      <CustomDataTable items={filteredStats} defaultSortColumn="name-column">
        ...
        <ColumnDefinition
          ...
          content={(q: QueueStats) => {
            const isMasked = isAgent && agentAssignedQueueSids.includes(q.queue.queue_sid);
            return <span>{isMasked ? '—' : q.tasks_now?.waiting_tasks}</span>;
          }}
        />
        <ColumnDefinition
          ...
          content={(q: QueueStats) => {
            const isMasked = isAgent && agentAssignedQueueSids.includes(q.queue.queue_sid);
            return <span>{isMasked ? '—' : q.workers?.total_available_workers}</span>;
          }}
        />
        <ColumnDefinition
